...
const Property = () => {
  ...
  // Create a wrapper function to properly pass the id parameter
  const fetchProperty = async (params?: { id: string }) => {
    if (!params?.id) throw new Error("Property ID is required")
    return await getPropertyById(params)
  }

  const { data: property, loading } = useAppwrite(fetchProperty, { id: id! }, { skip: !id })
  ...
  if (!property) {
    return (
      <View className="flex-1 justify-center items-center bg-white">
        <Text className="text-lg font-rubik-medium">Property not found</Text>
        <TouchableOpacity onPress={() => router.back()} className="mt-4 bg-primary-300 px-6 py-3 rounded-full">
              ...
              <View className="flex flex-row items-center justify-between mt-4">
                <View className="flex flex-row items-center">
                  ...
                  <View className="flex flex-col items-start justify-center ml-3">
                    <Text className="text-lg text-black-300 text-start font-rubik-bold">{property?.agent.name}</Text>
          ...
          {property?.gallery && property.gallery.length > 0 && (
            <View className="mt-7">
              ...
              <FlatList
                ...
                renderItem={({ item }) => <Image source={{ uri: item.image }} className="size-40 rounded-xl" />}
